            ...
            <fieldset class="scheduler-border">
                ...
                <div class="row control-group">
                        <div class="col-sm-2 form-group">
                            ...
                            <select class="filter" id="manufacturer" class="form-control" style="width:100% !important;">
                                ...
                                @forelse($manufacturers as $manu)
                                        <option value="{{$manu->id}}" @if(Request::get('manu') == $manu->id) selected @endif>{{$manu->name}}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
                            ...
                            <select class="filter" id="category" class="form-control" style="width:100% !important;">
                                ...
                                @forelse($categories as $category)
                                    ...
                                    @forelse($category->subcate as $sub )
                                        <option value="{{$sub->id}}" @if(Request::get('cate') == $sub->id) selected @endif>{{$sub->name}}</option>
                                    @empty
                                    @endforelse
                                @empty
                                @endforelse
                        ...
                        {{--<div class="col-sm-2 form-inline">--}}
                            {{--<div class="row">--}}
                                {{--<label for="" class="col-sm-5 plr-5">Sắp xếp:</label>--}}
                                {{--<div class="col-sm-7 plr-5">--}}
                                    {{--<div class="radio">--}}
                                        {{--<label><input type="radio" class="filter"  name="orderBy" value="acsending">&nbsp;Tăng dần</label>--}}
                                    {{--</div>--}}
                                    {{--<div class="radio">--}}
                                        {{--<label><input type="radio" class="filter" name="orderBy" value="descending" checked>&nbsp;Giảm dần</label>--}}
                                    {{--</div>--}}
                                {{--</div>--}}
                            {{--</div>--}}
                        {{--</div>--}}
                        <div class="col-sm-3 form-group">
                            <div class="row">
                                <label for="" class="col-sm-3 plr-5">Giá: <small>(VNĐ)</small></label>
